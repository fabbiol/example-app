            ...
            <main className="flex flex-1 flex-col items-center justify-center gap-6 p-6 text-center">
                ...
                <div>
                    <p className="text-sm font-medium uppercase tracking-wide text-emerald-800">
                        {completed ? 'Pedido concluído' : 'Carregamento registrado'}
                    </p>
                    <h1 className="mt-2 text-3xl font-bold">{loading.number}</h1>
                    <p className="mt-3 text-xl text-stone-700">
                        {loading.product?.name} · {formatQty(loading.quantity_m3)} m³
                    </p>
                    <p className="mt-2 text-lg text-stone-600">
                        ≈ {formatQty(loading.quantity_ton)} t
                        {' · '}
                        {formatQtyWithUnit(loading.quantity, loading.product?.unit)} no pedido
                    </p>
                    <p className="mt-1 text-base text-stone-500">Placa {loading.vehicle_plate}</p>
                </div>

                <div className="flex w-full max-w-md flex-col gap-3">
                    <Button asChild className="h-16 rounded-2xl bg-emerald-700 text-xl font-semibold text-white hover:bg-emerald-800">
